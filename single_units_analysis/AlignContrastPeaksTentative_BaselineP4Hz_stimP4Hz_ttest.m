...
for i = channum
    ...
    trindex =1:length(data_file.good_data(i).channel_data.sdftr_chan(1,:));
    power = nan(length(trindex),513);
    freq = nan(length(trindex),513);
    fourhzpower =nan(length(trindex),1);
    
    nyq = 15000;
    raw_bs = nan(length(1:600), length(trindex));
    filtered_dSUA = nan(length(1:600), length(trindex));
    bsl = nan(1, length(trindex));
    
    for tr = trindex
    data = data_file.good_data(i).channel_data.sdftr_chan(1:600,tr);
   
    lpc       = 100; %low pass cutoff
    lWn       = lpc/nyq;
    [bwb,bwa] = butter(4,lWn,'low');
    lpdSUA      = filtfilt(bwb,bwa, data(:));
        [power(tr,:), freq(tr,:)] = calcFFT(squeeze(data));
         ...
         %fourhzpower
          ...
          fourhzpower(tr,1) = power(tr,index);
    end
  eval(['all_power.dat_' filefieldname '_tad.power =fourhzpower;'])
  eval(['all_power.dat_' filefieldname '_tad.std_power = std(fourhzpower);'])
  eval(['all_power.dat_' filefieldname '_tad.mean_power = mean(fourhzpower);'])
...
eval(['cont_data_fieldnames = fieldnames(cont_data.cont' num2str(c) '_data);']) 
channum = 1:numel(cont_data_fieldnames);
power_fieldnames = fieldnames(all_power);
...
mean_data_file_power = nan(length(channum),1);
mean_fourhzpowerc = nan(length(channum),1);
...
eval(['trialidx = 1:length(cont_data.cont' num2str(c) '_data.' fieldname '.sdftr_chan(1,:));']);
    powerc = nan(length(trialidx),1025);
    freqc = nan(length(trialidx),1025);
    fourhzpowerc =nan(length(trialidx),1);
   ...
   for tridx = trialidx
    ...
    [bwb,bwa] = butter(4,lWn,'low');
    lpdSUA      = filtfilt(bwb,bwa, data);
    lpdSUA_bs      = filtfilt(bwb,bwa, raw_bs(:,tridx));
...
%}
    ...
    filtered_dSUA(:,tridx) = lpdSUA_bs;
    [powerc(tridx,:), freqc(tridx,:)] = calcFFT(lpdSUA(200:1350));
        
    %find the index of the frequency vector closest to 4hz and point to the
    %power value of this index for every trial, and store the value in
    %fourhzpower
    [val,index] = min(abs(4-freqc(tridx,:)));
    fourhzpowerc(tridx,1) = powerc(tridx,index);
   end
    %{
...
hold on
   ...
   for nb = 1:numel(power_fieldnames)
      power_fieldname = char(power_fieldnames(nb));
      underscores2 = strfind(power_fieldname, '_');
      splitfieldname2 = power_fieldname(underscores2(1)+1:underscores2(5)-1);

       if strcmp(splitfieldname, splitfieldname2) ==1
         eval(['data_file_power = all_power.dat_' splitfieldname2 '_tad.power;'])
         %eval(['data_file_std_bsl = all_bsls.dat_' splitfieldname2 '_tad.std_bsls_bslcorr;'])
       
       end
   end 

   [signi(i,1), pvalue] = ttest2(data_file_power,fourhzpowerc);
   mean_data_file_power(i,1) = mean(data_file_power);
   mean_fourhzpowerc(i,1) = mean(fourhzpowerc);
   if signi(i,1) == 1  && mean_data_file_power(i,1) < mean_fourhzpowerc(i,1)
 ...
 for n = 1:length(channum)
      ...
      upper_bound =round(max_low_dist)-round(all_locsdSUA_filtered(n))+length(xabs);
        %if signi(n,1) ==1 && mean_data_file_power(n,1) < mean_fourhzpowerc(n,1)
...
x = 1:length(fp_locked_data(:,1));
 plot(x, fp_locked_data(:,:),'HandleVisibility','off')
 hold on
 plot(x, mean(fp_locked_data(:,:),2),'LineWidth',1, 'Color', 'black')
 plot(xlim, [0 0],'k','HandleVisibility','off')
 %plot(xlim, [mean(data_file_power) mean(data_file_power)],'k')
 %plot(xlim, [mean(data_file_bsl)+1.96*data_file_std_bsl mean(data_file_bsl)+1.96*data_file_std_bsl],'k')
 
  %txtmeanBSL = ['mean baseline'];
  %text(0, mean(data_file_power)+3, txtmeanBSL)
